...
public class SearchByPlayerNameController{
    ...
    void activateSearch(ActionEvent event) {
        ...
        String name = searchTextFieldID.getText();
        if(name.isEmpty()){
            main.showAlert("Invalid Input","Player Name Text Field Is Empty","Please input data correctly");
            buttonClicked = false;
            result = true;
        }
        else {
            Players players = Option1MainMenu.searchByPlayerName(name);
            if(players.getName().equalsIgnoreCase("Not Found")){
                main.showAlert("Invalid Input","No Such Player With This Name. ","Please input player's name correctly");
                result = false;
            }
            else {
                list.clear();
