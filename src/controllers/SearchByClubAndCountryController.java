...
public class SearchByClubAndCountryController{
    ...
    void activateSearch(ActionEvent event) {
        ...
        String countryName = searchTextFieldID.getText();
        if(countryName.isEmpty()){
            main.showAlert("Invalid Input","Country Name Text Field Is Empty","Please input data correctly");
            buttonClicked = false;
            result = true;
        }
        else {
            List<Players> tempList = Option1MainMenu.searchByClubAndCountry(countryName);
            if(tempList.isEmpty()){
                main.showAlert("Invalid Input","No Such Player With This Country. ","Please input country Name correctly");
                result = false;
            }
            else {
                list.clear();
